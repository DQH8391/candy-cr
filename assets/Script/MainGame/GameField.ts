import { _decorator, Component, instantiate, log, Prefab, tween } from 'cc';
...
export class GameField extends Component {
 ...
 //todo 
  ...
  private needCheckFieldAfterSwapCircle() {
        ...
        this.node.dispatchEvent(new Event.EventCustom('needCheckField', true));
        ...
        log("countProgressStep")
        this.node.dispatchEvent(new Event.EventCustom('setUnBlockTouch', true));
        this.node.dispatchEvent(new Event.EventCustom('countProgressStep', true));
        this.prewCell = null;
        this.currentCell = null;
        this.tmpPrewCell = null;
        }
  ...
  private validateCircleMove(i, j, newi, newj) {
        ...
        if (CheckerBoolean.checkTwoBoolean(this.Cells[j][i]._circle == null, this.Cells[newj][newi]._circle != null))
         return true;
        return false;
  }
  ...
  private fieldOnFilledWithCircles() {
        ...
        log(allcirclenowinfield);
        log(allcirclenowinfieldandmove);
  ...
  private createRainbowBall(i, j, iOne, jOne, iTwo, jTwo, iThree, jThree, iFour, jFour, tipe) {
        ...
        log(this.Cells[j][i]._circle.getComponent(Circle).CircleType);
  ...
  private destroyEveryCircles() {
        ...
        this.scheduleOnce(function () {
        this.node.dispatchEvent(new Event.EventCustom('destroyCircles', true));
        }, this.iter + 0.1);
        this.scheduleOnce(function () {
        this.node.dispatchEvent(new Event.EventCustom('needCheckField', true));
        }, (this.iter + 0.2) * 4);
  ...
  getTypeDestroyCircle(circle) {
        ...
        log(this.destroyTipeColors[circle.CircleTypeColor]);
